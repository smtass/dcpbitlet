...
public class Metafile extends Bencode {
    ...
    public Metafile(InputStream is) throws IOException, NoSuchAlgorithmException {
        ...
        name = new String(((ByteBuffer) info.get(toByteBuffer("name"))).array(),"UTF8");
        /*
        byte [] a = (((ByteBuffer)(info.get(toByteBuffer("name"))))).array();
        byte [] b = name.getBytes();
        for(int tmp = 0; tmp < a.length; tmp++){
            System.out.print(Integer.toHexString(a[tmp] & 0xFF));
        }
        System.out.println();
        for(int tmp = 0; tmp < b.length; tmp++){
            System.out.print(Integer.toHexString(b[tmp] & 0xFF));
        }
        System.out.println();
        System.out.println("name: " + new String(
                new String(
                        (((ByteBuffer)(info.get(toByteBuffer("name"))))).array(),"UTF8").getBytes(),
                        "GBK")
        );
        System.out.println("name: " + new String(name.getBytes(),"GBK"));
        //System.out.println("name: " + new String(name.getBytes("UTF8"),"GBK"));
        */
